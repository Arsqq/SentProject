...
import com.example.eurekaclient1.Repo.RoleRepository;
...
import com.example.eurekaclient1.payload.request.LoginRequest;
...
public class AuthController {
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
		...
		user.get().setDateOfLastLogOn(date);
		userRepository.save(user.get());
		return ResponseEntity.ok(new JwtResponse(jwt,
												 userDetails.getId(), 
												 userDetails.getUsername(), 
												 userDetails.getEmail(),
												 roles,userDetails.getDateOfLastLogOn()));
	}
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
		...
		user.setRoles(roles);
		user.setDateOfLastLogOn(new Date());
